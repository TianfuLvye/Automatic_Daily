...
当时的 `docker-compose.yml` 只有 RSSHub 和 Redis;DailyHot 靠 README 里一条 `docker run`。主程序不在 Compose 里,换机器或重建容器没有单一入口。容器网络里 `127.0.0.1` 是 Automatic Daily 自己,不是 RSSHub。

## 决策

1. **Automatic Daily 作为 compose 服务跑 `main.py serve`。** `./data` 和 `./config` bind mount。重建容器不丢库、不丢报纸、不丢 `sources.yaml`。
...
- **负面**: 镜像含 Chromium 和 CJK 字体,体积大、首次构建慢。本机再开一份 `serve` 会和容器抢同一份 SQLite,需要先 `docker compose stop automatic-daily`。
