...
| 「新增才推送」的增量心智 | 已有 `newly_entered` / 快照表；关键词过滤叠在加工层 |
| 调度分区（采集 vs 推送） | 由 APScheduler / CLI 分别调用 |
...
2. **依赖链已经够长**  
   TrendRadar 热榜默认走 NewsNow：`平台 → NewsNow → TrendRadar → 我们`。可用性乘性衰减，故障归因也难。我们已是 `平台 → DailyHotApi → 我们`（少一环）。再套 TrendRadar 是开倒车。
...
5. **时间盒**
   只把 TrendRadar 当作参考，不把它变成运行时依赖。

## 后果

- **正面**: 关键词与热榜采集可独立演进；少一环依赖；版面 `sections` 是我们独有、TrendRadar 没有的字段。
- **负面**: 推送渠道（飞书/TG）另行实现；AI 分析/漂亮 HTML 报告暂不复用。
- **风险**: 关键词召回有上限（同义/黑话）——已用 `aliases` 打底，向量召回补洞。
